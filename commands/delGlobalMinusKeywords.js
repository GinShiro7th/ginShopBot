...
module.exports = async function(msg, bot, option, fromId){
  ...
  switch (option){
    case '1':
      await bot.sendMessage(msg.chat.id, "🔎Пришлите минус слова, которые хотите удалить из глобальных");
      await user.update({
    ...
    case '2':
      ...
      await bot.sendMessage(msg.chat.id, "✅Минус слова успешно удалены из глобальных");
      await user.update({
        Command: "start"
      });
      break;  
